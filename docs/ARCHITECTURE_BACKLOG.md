...
| `@riverpod` codegen migration | **Open** | Conditions, app shell, map, hydro done (#20–#21, #23); **`goRouterProvider` still manual** in `packages/routing/` |
...
| Result-based providers (conditions UI layer) | **Done** | Providers throw `AppFailure`; launch detail reads typed `AsyncError` |
| Result-based boundaries (map / hydro) | **Open** | Map throws `StateError`; hydro propagates raw parse/load exceptions |
| Firebase callables (conditions) | **Done** | Callables return `Result`; repos pass-through |
