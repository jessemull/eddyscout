...
class ConditionsService {
  ConditionsService({EddyScoutHttpClient? http}) : _http = http ?? EddyScoutHttpClient();
  ...
  Future<ConditionsSnapshot> load(LaunchPoint launch) async {
    ...
    final weatherFuture = _loadWeather(launch.latitude, launch.longitude, fetchedAt);
  ...
  Future<_WeatherResult> _loadWeather(double lat, double lon, DateTime now) async {
    ...
    try {
      ...
      if (pointsJson == null) {
        return openMeteoOrError('NWS location lookup failed; Open-Meteo had no data.');
      }
      final hourlyUri = nwsHourlyForecastUriFromPoints(pointsJson);
      if (hourlyUri == null) {
        return openMeteoOrError('NWS hourly URL missing; Open-Meteo had no data.');
      }
      final hourlyJson = await _http.getNwsJson(hourlyUri);
      if (hourlyJson == null) {
        return openMeteoOrError('NWS hourly forecast failed; Open-Meteo had no data.');
      }
      final parsed = weatherFromNwsHourly(hourlyJson, now: now);
      if (parsed == null) {
        return openMeteoOrError('Could not parse NWS hourly; Open-Meteo had no data.');
      }
      return _WeatherResult(parsed, null);
    } catch (e) {
      try {
        return await openMeteoOrError('NWS error ($e); Open-Meteo had no data.');
      } catch (e2) {
  ...
  Future<_TideResult> _loadTides(LaunchPoint launch) async {
    if (launch.tideRelevance == TideRelevance.none || launch.noaaTideStationId == null) {
    ...
    Future<TideSummary?> tryDatum(String datum) async {
      final uri = Uri.https('api.tidesandcurrents.noaa.gov', '/api/prod/datagetter', {
        'product': 'predictions',
  ...
  Future<_RiverResult> _loadRiverFlow(LaunchPoint launch) async {
    ...
    try {
      ...
      return _RiverResult(reading, reading == null ? 'No discharge (cfs) at this site right now.' : null);
    } catch (e) {
